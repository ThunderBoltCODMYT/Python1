...
ENEMY_START_Y_MIN, ENEMY_START_X_MAX = 50, 150
#Initialize Pygame
pygame.init()
#Create the screen
...
playerImg = pygame.image.load('player.png.jpeg')
...
for i in range(num_of_enemies):
    enemyImg.append(pygame.image.load('enemy.png.jpeg'))
    enemyX.append(random.randint(0, SCREEN_WIDTH - 64))
    enemyY.append(random.randint(ENEMY_START_Y_MIN , ENEMY_START_X_MAX))
    enemyX_change.append(ENEMY_SPEED_X)
    enemyY_change.append(ENEMY_SPEED_Y)
#Bullet
bulletImg = pygame.image.load('bullet.png.jpeg')
bulletX = 0
...
def fire_bullet(x, y):
    global bullet_state
    bullet_state = 'fire'
    screen.blit(bulletImg, (x + 16, y + 10))  # Adjust bullet position to center it on the player
...
for i in range(num_of_enemies):
    ...
    if isCollision(enemyX[i], enemyY[i], bulletX, bulletY):
        ...
        enemyY[i] = random.randint(ENEMY_START_Y_MIN, ENEMY_START_X_MAX)
